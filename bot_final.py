...
def ejecutar_caza_asimetrica(direccion, precio_mercado, fuerza_senal):
    ...
    try:
        ...
        # CORRECCIÓN EN ÓRDENES DE SALIDA: Parámetro closePosition pasado como string y precios de activación formateados como cadenas de texto
        binance_client.futures_create_order(symbol=SYMBOL, side=side_salida, type='TAKE_PROFIT_MARKET', stopPrice=str(precio_tp), closePosition='true')
        binance_client.futures_create_order(symbol=SYMBOL, side=side_salida, type='STOP_MARKET', stopPrice=str(precio_sl), closePosition='true')
...
def webhook():
    ...
    # CORRECCIÓN DE CONDICIONAL INVERSO: Si la función devuelve False (hay mechazo), entra a cancelar. Si devuelve True (estabilidad), avanza.
