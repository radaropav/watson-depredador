...
def ejecutar_caza_asimetrica(client_local, direccion, precio_mercado, fuerza_senal):
    ...
    try:
        ...
        client_local.futures_create_order(symbol=SYMBOL, side=side_salida, type='TAKE_PROFIT_MARKET', stopPrice=precio_tp, quantity=quantity)
        client_local.futures_create_order(symbol=SYMBOL, side=side_salida, type='STOP_MARKET', stopPrice=precio_sl, quantity=quantity); guardar_auditoria_supabase(direccion, precio_mercado)
